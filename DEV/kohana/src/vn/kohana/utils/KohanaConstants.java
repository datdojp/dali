package vn.kohana.utils;

public interface KohanaConstants {
	/** CLIENT PAGES **/
	public static final String PAGE_CLIENT_HOMEPAGE = "/pages/client/HomePage.jsf";
	public static final String PAGE_CLIENT_SHOW_CATEGORY = "/pages/client/ShowCategory.jsf";
	public static final String PAGE_CLIENT_SEARCH_RESULTS = "/pages/client/SearchResults.jsf";
	public static final String PAGE_CLIENT_SHOW_CART = "/pages/client/ShowCart.jsf";
	public static final String PAGE_CLIENT_ORDER = "/pages/client/Order.jsf";
	
	/** ADMIN PAGES **/
	public static final String PAGE_ADMIN_MANAGE_PRODUCT = "/pages/admin/ManageProduct.jsf";
	public static final String PAGE_ADMIN_CREATE_PRODUCT = "/pages/admin/CreateProduct.jsf";
	public static final String PAGE_ADMIN_EDIT_PRODUCT = "/pages/admin/EditProduct.jsf";
	public static final String PAGE_ADMIN_MANAGE_ORDER = "/pages/admin/ManageOrder.jsf";
	public static final String PAGE_ADMIN_VIEW_ORDER = "/pages/admin/ViewOrder.jsf";
	
	/** MESSAGES **/
	public static final String MSG_COMMON_ERROR = "Có lỗi xảy ra. Thao tác không thực hiện được";
	public static final String MSG_MISSING_MANDATORY_FIELD = "Hãy nhập thông tin cho ";
}
